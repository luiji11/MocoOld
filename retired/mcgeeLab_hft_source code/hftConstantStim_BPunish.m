function hftConstantStim_BPunish(SubjectId, sessionDay)
...
%% GO
    ...
    while (GetSecs - tStart < int_rsp) % int_rsp sec to respond        
        ...
        if response == 0 && mouseLicked  && (GetSecs - tStart > 2); % if HIT
            response = 1; 
            maeFeedPorts(S,rewardPort);
            I.water = maeStoreEvents(I.water, K.k, rewardPort);           
            tOver = GetSecs;
            while (GetSecs - tOver) < 4; [go, K.k, I.lick, ~] = maeScan(K,go,I,F,S); end % wait .75 s                        
