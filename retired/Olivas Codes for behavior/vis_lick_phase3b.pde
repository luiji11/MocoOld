...
int preRespWindInt= 4*60;
int respWindInt   = 2*60;
...
//  size(displayWidth, displayHeight, P2D);
  ...
  output = createWriter("phase3bTESTING.txt"); 
...
background(128);
  ...
  {  
    sOn = frameCount + 10*60;//int(random(30, 60)); 
   ...
   if (frameCount>sOff-respWindInt-preRespWindInt){
        myPort.write('w');      
...
//        print(isLicking);         
        ...
        if (frameCount <=  sOff-respWindInt){
        ...
        if (frameCount >  sOff-respWindInt){
          ...
          if (rewardGiven == 0 && isLicking == 1 && lastThree[2]==0 && noiseLicks==0) {
            print("*");
            myPort.write('d');
            rewardGiven = 1; 
